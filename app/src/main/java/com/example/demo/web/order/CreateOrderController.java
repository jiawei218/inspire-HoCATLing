...
public class CreateOrderController {
        ...
        public record CreateOrderRequest(
                        ...
                        @Valid DiscountParaRequest discountPara) {
                public record DiscountParaRequest(
                                @NotNull(message = "折扣类型不能为空") String type,
                                @NotNull(message = "折扣门槛不能为空") @Min(0) BigDecimal threshold,
                                @NotNull(message = "折扣金额不能为空") @DecimalMin(value = "0", inclusive = false, message = "折扣金额必须大于0") BigDecimal amount) {
                }
                ...
                public record DeliveryInfoRequest(
                                ...
                                @NotNull(message = "收货人手机号不能为空") @Pattern(regexp = "^1[3-9]\\d{9}$", message = "手机号格式不正确") String recipientPhone,
        ...
        public record CreateOrderResponse(int code, String message, OrderData data) {
                public record OrderData(
                                String orderId, String orderNumber, String status, PricingData pricing,
                                String createdAt) {
                }

                public record PricingData(
                                BigDecimal itemsTotal, BigDecimal packagingFee, BigDecimal deliveryFee,
                                BigDecimal finalAmount, DiscountData discountData) {
                }

                public record DiscountData(
                                boolean hasDiscount, String description, BigDecimal amount, String type) {
                }
        }
        ...
        private CreateOrderCommand toCommand(CreateOrderRequest request, User user) {
                ...
                CreateOrderCommand.DiscountParaDto discountPara = null;
                if (request.discountPara() != null) {
                        DiscountType discountType = DiscountType.valueOf(request.discountPara().type());
                        discountPara = new CreateOrderCommand.DiscountParaDto(
                                        discountType,
                                        request.discountPara().threshold(),
                                        request.discountPara().amount());
                }
        ...
        private CreateOrderResponse toResponse(CreateOrderResult result) {
                CreateOrderResponse.PricingData pricingData = new CreateOrderResponse.PricingData(
                                ...
                                new CreateOrderResponse.DiscountData(
                                                result.pricing().discountInfo().hasDiscount(),
                                                result.pricing().discountInfo().description() != null
                                                                ? result.pricing().discountInfo().description()
                                                                : "",
                                                result.pricing().discountInfo().amount(),
                                                result.pricing().discountInfo().type() != null
                                                                ? result.pricing().discountInfo().type().name()
                                                                : ""));
