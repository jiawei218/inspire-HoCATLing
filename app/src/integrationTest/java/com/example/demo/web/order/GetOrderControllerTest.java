...
class GetOrderControllerTest {
        ...
        @Test
        void get_order_should_return_discount_info_when_order_has_discount() throws Exception {
                String orderId = createTestOrderWithDiscount("test-user-003", "FULL_REDUCTION", new BigDecimal("50.00"),
                                new BigDecimal("20.00"));
                mockMvc.perform(get("/api/v1/orders/{orderId}", orderId).with(user("test-user-003")))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.code").value(0))
                                .andExpect(jsonPath("$.data.orderId").value(orderId))
                                .andExpect(jsonPath("$.data.pricing.itemsTotal").value(50.00))
                                .andExpect(jsonPath("$.data.pricing.finalAmount").value(34.00))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.hasDiscount").value(true))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.description").value("满50减20"))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.amount").value(20.00))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.type").value("FULL_REDUCTION"));

        }

        @Test
        void get_order_should_return_discount_info_when_order_has_no_discount() throws Exception {
                String orderId = createTestOrderWithoutDiscount("test-user-005");

                mockMvc.perform(get("/api/v1/orders/{orderId}", orderId).with(user("test-user-005")))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.code").value(0))
                                .andExpect(jsonPath("$.data.orderId").value(orderId))
                                .andExpect(jsonPath("$.data.pricing.itemsTotal").value(50.00))
                                .andExpect(jsonPath("$.data.pricing.finalAmount").value(54.00))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.hasDiscount").value(false))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.description").value(""))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.amount").value(0.00))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.type").value(""));
        }

        @Test
        void get_order_should_return_discount_info_when_discount_not_applied_due_to_threshold() throws Exception {
                String orderId = createTestOrderWithDiscount("test-user-003", "FULL_REDUCTION",
        ...
        @Test
        void get_order_should_return_discount_info_with_small_amount() throws Exception {
                String orderId = createTestOrderWithDiscount("test-user-003", "FULL_REDUCTION", new BigDecimal("50.00"),
                ...
                mockMvc.perform(get("/api/v1/orders/{orderId}", orderId).with(user("test-user-003")))
                                ...
                                .andExpect(jsonPath("$.data.orderId").value(orderId))
                                .andExpect(jsonPath("$.data.pricing.itemsTotal").value(50.00))
                                .andExpect(jsonPath("$.data.pricing.finalAmount").value(53.965))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.hasDiscount").value(true))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.description").value("满50减0.035"))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.amount").value(0.035))
                                .andExpect(jsonPath("$.data.pricing.discountInfo.type").value("FULL_REDUCTION"));
        }
        ...
        /**
         * Helper method to create a test order without discount.
         */
        private String createTestOrderWithoutDiscount(String userId) throws Exception {
                CreateOrderController.CreateOrderRequest request = new CreateOrderController.CreateOrderRequest(
                                "merchant-001",
                                List.of(new CreateOrderController.CreateOrderRequest.OrderItemRequest(
                                                "dish-001", "宫保鸡丁", 2, new BigDecimal("25.00"))),
        ...
        /**
         * Helper method to create a test order with discount.
         */
        private String createTestOrderWithDiscount(String userId, String discountType, BigDecimal threshold,
                        BigDecimal amount) throws Exception {
                CreateOrderController.CreateOrderRequest.DiscountParaRequest discountPara = new CreateOrderController.CreateOrderRequest.DiscountParaRequest(
                                discountType, threshold, amount);
                CreateOrderController.CreateOrderRequest request = new CreateOrderController.CreateOrderRequest(
                                "merchant-001",
                                List.of(new CreateOrderController.CreateOrderRequest.OrderItemRequest(
                                                "dish-001", "宫保鸡丁", 2, new BigDecimal("25.00"))),
